...
import { BEUniverses, PlayState, PlayStateDescription } from "./lib/Types";
...
export const Universes = ({ walletAddressStr, onUniversesChange, isGameStarted, gameState, playStateDescription, startGame }: UniversesProps) => {
    ...
    return (
        <>
            <Row wrap={false} className="universes-row">
                ...
                <Col>
                    {universes.wonTonPower === 0 ?
                     <PlayTon universes={universes}
                              isGameStarted={isGameStarted}
                              playStateDescription={playStateDescription}
                              walletAddressStr={walletAddressStr}
                              startGame={startGame}/> :
                     <PlayNft key={`${walletAddressStr}-${universes.wonTonPower}`}
                              universes={universes}
                              isGameStarted={isGameStarted}
                              playStateDescription={playStateDescription}
                              walletAddressStr={walletAddressStr}
            ...
            <Row wrap={false} className="universes-row">
                <Col flex={'auto'}>
                    {gameState == "WIN" ? (<div className="sub-status-win">You Won last play. Congratulations!!!</div>) : null}
                    {gameState == "LOOSE" ? (<div className="sub-status-loose">You Loose last play. Cheer up!!!</div>) : null}
                    {isGameStarted && isGameTakingTooLong ? (<div className="sub-status-generic">It can take much longer than expected, as your <b>Game</b> depends on other player[s]...</div>) : null}
                </Col>
            </Row>
